...
function Principal() {
    ...
    const opcionesDeAno = [ "ELIGE UNA OPCION", "2022", "2023", "2024", "2025", "2026", "2027", "2028" ];
        ...
        <div>
            <form className={estilos.contenedor}>
                <label className={estilos.label}>
                    ...
                    >
                        {opcionesDeAno.map(opcion => <option value={opcion}>{opcion}</option>)}
                    </select>
                </label>
                <label className={estilos.label}>
                    ...
                    >
                            {opcionesDeMonto.map(opcion => <option value={opcion}>{opcion}</option>)}
                    </select>
                </label>
                <label className={estilos.label}>
                    Ingresa el monto...
            ...
            >
                <p 
                className={estilos.textoResultado}
                >El valor <strong>{tipoMonto === "BRUTO" ? 'LIQUIDO' : 'BRUTO'}</strong> de tu Boleta de Honorarios es:</p>
                <h3 
                className={estilos.resultado}
                ><strong>${tipoMonto === "LIQUIDO" ? valorBruto : valorLiquido}.-</strong></h3>
                <div className={estilos.pie}>
                <p>Valor de acuerdo a la ley 21.133 que aumenta gradualmente la retención de impuestos para Boletas de Honorarios</p>
                </div>
            </Modal>
        </div>
    );
